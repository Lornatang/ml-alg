...
class NeuralNet():
    ...
    def __init__(self, n_inputs, n_outputs, n_hidden):
        ...
        # Init all paras
        self.W_h = np.random.randn(self.inputs, self.hidden)
        self.b_h = np.zeros((1, self.hidden))
        self.W_o = np.random.randn(self.hidden, self.outputs)
        self.b_o = np.zeros((1, self.outputs))
    ...
    def backward(self,  X, Y, n_samples, outputs):
        """
    ...
    def train(self, X, y, n_iters=500, eta=0.3):
        ...
        for i in range(n_iters):
            ...
            gradients = self.backward(X, y, n_samples, outputs)
            if i % 100 == 0:
